...
SonarQube Cloud automatically analyzes pull requests for this repository and
publishes the `SonarCloud Code Analysis` check. The active `main-production`
ruleset requires that check from the SonarQube Cloud GitHub App, so a pull
request cannot merge unless its SonarQube quality gate passes.
