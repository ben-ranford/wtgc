...
(cd "$positive" && SUPPRESSION_BASE=HEAD "$gate")
 (cd "$positive" && expect_failure env SUPPRESSION_BASE=HEAD SUPPRESSION_VERIFY_ISSUES=1 GITHUB_REPOSITORY=example/repo "$gate")
git -C "$positive" add .
...
(cd "$duplicate" && expect_failure env SUPPRESSION_BASE=HEAD "$gate")
